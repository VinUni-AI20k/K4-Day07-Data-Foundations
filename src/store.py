...
class EmbeddingStore:
    ...
    def __init__(
        ...
        try:
            ...
            # Ephemeral Chroma clients may share an in-process system.  A unique
            # physical name keeps each EmbeddingStore instance isolated, while
            # _collection_name retains the caller's logical name.
            self._collection = client.create_collection(name=f"lab-{uuid4().hex}")
    ...
    def _get_chroma_records(self) -> list[dict[str, Any]]:
        """Load normalized records from Chroma for consistent dot-product ranking."""
