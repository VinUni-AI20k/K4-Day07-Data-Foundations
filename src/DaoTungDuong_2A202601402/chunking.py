...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Strip extra whitespace from each sentence, filtering out empty ones
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # Fallback when no separators left: split by character size
        ...
        for piece in pieces:
            ...
            if current_len + separator_len + len(piece) <= self.chunk_size:
                current_chunk_parts.append(piece)
                current_len += separator_len + len(piece)
            else:
                if current_chunk_parts:
                    merged_chunks.append(separator.join(current_chunk_parts))
